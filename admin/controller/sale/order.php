...
class ControllerSaleOrder extends Controller {
	...
	public function upload() {
  	...
  	public function invoice() {
		...
		$this->data['logo'] = HTTP_CATALOG . 'image/invoice/park-invoice_logo.jpg';		
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$format = '<span>Name:</span>{firstname} {lastname}' . "\n" . '<span>Company:</span>{company}' . "\n" . '<span>Address:</span>{address_1}' . "\n" . '<span>Address 2:</span>{address_2}' . "\n" . '<span>City:</span>{city}' . "\n" . '<span>State:</span>{zone}' . "\n". '<span>Zip Code:</span>{postcode}' . "\n" . '<span>Country:</span>{country}';
				...
				foreach ($products as $product) {
					...
					foreach ($options as $option) 
					{
						...
						if(isset($product_info['has_option']) && ($product_info['has_option'] == 1))
						{
                    		 $stock_qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_option_relation` WHERE `var` = '".$option['product_option_value_id']."' AND `product_id` = '".(int)$product['product_id']."' LIMIT 1");
							 $sku = $stock_qry->row['sku'];
    	                }
				...
				{
					if ($order_info['order_status_id'] == $status['order_status_id']) $order_status = $status['name'];
				}
				...
				$this->data['orders'][] = array(
					...
					'text_greeting' 	 => sprintf($this->language->get('text_new_greeting'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'))
				);
			}
		}
	...
	{		
		...
		$this->data['store_name'] = $order_info['store_name'];
		$this->data['logo'] = HTTP_CATALOG .  $this->config->get('config_url') . 'image/' . $this->config->get('config_logo');		
		$this->data['store_url'] = $order_info['store_url'];
		$this->data['order_id'] = $order_id;
		$this->data['date_added'] = date($language->get('date_format_short'), strtotime($order_info['date_added']));    	
		$this->data['payment_method'] = $order_info['payment_method'];
		$this->data['shipping_method'] = $order_info['shipping_method'];
		...
		$this->data['payment_zone_code'] =  $order_info['payment_zone_code'];
		$this->data['payment_country'] =  $order_info['payment_country'];
		$this->data['payment_phone'] =  $order_info['telephone'];
		...
		$this->data['shipping_zone_code'] =  $order_info['shipping_zone_code'];
		$this->data['shipping_country'] =  $order_info['shipping_country'];
		$this->data['shipping_phone'] =  $order_info['telephone'];
		
		$this->data['store_address'] = $this->config->get('store_address');
		$this->data['store_phone'] = $this->config->get('config_telephone');
		...
		$order_product_query = $this->db->query("SELECT o.*, p.sku FROM " . DB_PREFIX . "order_product o JOIN " . DB_PREFIX . "product p ON p.product_id = o.product_id WHERE order_id = '" . (int)$order_id . "'");
		...
		$this->data['totals'] = $order_total_query->rows;	
		
		$this->template = 'sale/order_receipt.tpl';
