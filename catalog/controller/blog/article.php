      ...
      // Redirect page if Blog Manager is not installed
		...
		if ($articleData) {
         ...
         $imageHeight = $blogSetting['articleFeatHeight'];
			...
			$this->data['image'] = $this->model_tool_image->resize($articleData['image'], 912, 322);
			...
			if (isset($blogSetting['artInfoDate'])) {
				$art_InfoDate = sprintf($this->language->get('text_art_InfoDate'), $this->model_blog_helper->date_format($articleData['created'], 'dfs'));
			} else {
				$art_InfoDate = '';
			}
