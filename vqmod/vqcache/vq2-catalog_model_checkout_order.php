...
class ModelCheckoutOrder extends Model {	
	...
	public function addOrder($data) {
		...
		$this->db->query("UPDATE ".DB_PREFIX."order SET invoice_no = " . $order_id . " WHERE order_id = " . $order_id);
...
// template email - start
            ...
            foreach ($order_product_query->rows as $order_product){
                    ...
                    if(isset($product_query->row['has_option']) && ($product_query->row['has_option'] == 1)){
                            ...
                            AND ((`" . DB_PREFIX . "option`.`type` = 'radio') OR (`" . DB_PREFIX . "option`.`type` = 'select') OR (`" . DB_PREFIX . "option`.`type` = 'image') OR (`" . DB_PREFIX . "option`.`type` = 'color'))
...
// template email - start
	...
	private function createInvoiceNo($order_id) 
	{
		$order_info = $this->getOrder($order_id);
