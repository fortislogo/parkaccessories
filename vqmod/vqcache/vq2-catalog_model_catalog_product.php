...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *,(SELECT CASE WHEN p2p.product_id IS NOT NULL THEN MIN(pr2.quantity - p2p.quantity)+1 ELSE p.quantity END FROM " . DB_PREFIX . "product_to_product p2p INNER JOIN " . DB_PREFIX . "product AS pr2 ON p2p.related_id = pr2.product_id WHERE p2p.product_id = p.product_id) As quantity, IFNULL((SELECT 1 FROM " . DB_PREFIX . "product p2 INNER JOIN " . DB_PREFIX . "product_to_product p2p ON p2.product_id = p2p.product_id WHERE p.product_id = p2p.related_id AND p2.product_bundle_hidden = 1 LIMIT 1),0) As product_hidden, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
...
'product_hidden'    => $query->row['product_hidden'],
				...
				'custom_title_1'   => $query->row['custom_title_1'],
				'custom_title_2'   => $query->row['custom_title_2']
			);
		} else {
			return false;
		}
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND (SELECT 1 FROM " . DB_PREFIX . "product p2 INNER JOIN " . DB_PREFIX . "product_to_product p2p ON p2.product_id = p2p.product_id WHERE p.product_id = p2p.related_id AND p2.product_bundle_hidden = 1 LIMIT 1) IS NULL AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
	...
	public function getLatestProducts($limit) {
		...
		if (!$product_data) { 
			$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND (SELECT 1 FROM " . DB_PREFIX . "product p2 INNER JOIN " . DB_PREFIX . "product_to_product p2p ON p2.product_id = p2p.product_id WHERE p.product_id = p2p.related_id AND p2.product_bundle_hidden = 1 LIMIT 1) IS NULL AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT " . (int)$limit);
	...
	public function getPopularProducts($limit) {
		...
		$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND (SELECT 1 FROM " . DB_PREFIX . "product p2 INNER JOIN " . DB_PREFIX . "product_to_product p2p ON p2.product_id = p2p.product_id WHERE p.product_id = p2p.related_id AND p2.product_bundle_hidden = 1 LIMIT 1) IS NULL AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.viewed, p.date_added DESC LIMIT " . (int)$limit);
	...
	public function getBestSellerProducts($limit) {
		...
		if (!$product_data) { 
			...
			$query = $this->db->query("SELECT op.product_id, COUNT(*) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND (SELECT 1 FROM " . DB_PREFIX . "product p2 INNER JOIN " . DB_PREFIX . "product_to_product p2p ON p2.product_id = p2p.product_id WHERE p.product_id = p2p.related_id AND p2.product_bundle_hidden = 1 LIMIT 1) IS NULL AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY op.product_id ORDER BY total DESC LIMIT " . (int)$limit);
...
//BOF Product Color Option
	...
	public function getProductRelated($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND (SELECT 1 FROM " . DB_PREFIX . "product p2 INNER JOIN " . DB_PREFIX . "product_to_product p2p ON p2.product_id = p2p.product_id WHERE p.product_id = p2p.related_id AND p2.product_bundle_hidden = 1 LIMIT 1) IS NULL AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
	...
	public function getTotalProductSpecials() {
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT ps.product_id) AS total FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND (SELECT 1 FROM " . DB_PREFIX . "product p2 INNER JOIN " . DB_PREFIX . "product_to_product p2p ON p2.product_id = p2p.product_id WHERE p.product_id = p2p.related_id AND p2.product_bundle_hidden = 1 LIMIT 1) IS NULL AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW()))");
